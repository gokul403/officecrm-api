...
router.post("/income", requireAuth, requireRole(["admin", "manager"]), async (req, res) => {
    ...
    try {
        const result = await pool.query(`INSERT INTO income (amount, source, category, description, customer_id, received_on, created_by)
       VALUES ($1, $2, $3, $4, $5, $6, $7)
       RETURNING *`, [amount, source, category || null, description || null, customer_id || null, received_on || new Date().toISOString().split("T")[0], createdBy]);
...
router.post("/expenses", requireAuth, requireRole(["admin", "manager"]), async (req, res) => {
    ...
    try {
        const result = await pool.query(`INSERT INTO expenses (amount, category, vendor, description, spent_on, paid_by, created_by)
       VALUES ($1, $2, $3, $4, $5, $6, $7)
       RETURNING *`, [amount, category, vendor || null, description || null, spent_on || new Date().toISOString().split("T")[0], paid_by || null, createdBy]);
        return res.status(201).json(result.rows[0]);
    }
    catch (error) {
