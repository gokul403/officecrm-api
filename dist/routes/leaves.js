...
router.post("/", requireAuth, async (req, res) => {
    ...
    `;
        ...
        try {
            const employeeRes = await pool.query("SELECT full_name, email FROM profiles WHERE id = $1", [user.id]);
...
router.put("/:id/status", requireAuth, async (req, res) => {
    ...
    `;
        ...
        try {
            const reviewerRes = await pool.query("SELECT full_name FROM profiles WHERE id = $1", [user.id]);
