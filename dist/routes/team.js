...
router.post("/team/password", requireAuth, requireRole(["admin"]), async (req, res) => {
    ...
    try {
        const userQuery = await pool.query(`SELECT u.id, u.password_hash, p.is_active
       FROM users u
...
router.post("/team/members", requireAuth, requireRole(["admin"]), async (req, res) => {
    ...
    try {
        ...
        if (role === "employee") {
            const managerCheck = await client.query(`SELECT p.id FROM profiles p
         INNER JOIN user_roles ur ON ur.user_id = p.id
         WHERE p.id = $1 AND ur.role = 'manager' AND p.is_active = TRUE`, [managerId]);
