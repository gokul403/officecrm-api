...
export function buildTaskAssignedMessage(task, assignerName) {
...
export function buildLeadCreatedMessage(lead, creatorName) {
    const lines = [
    ...
    if (lead.assignee_name)
        lines.push(`Assigned to: ${lead.assignee_name}`);
 ...
 */
export function toLocalPhoneDigits(phone) {
...
export function phonesMatchLocal(a, b) {
 ...
 */
export function toWhatsAppMsisdnDigits(phone) {
...
export function normalizePhoneToChatId(phone) {
...
export function chatIdToPhoneDigits(chatId) {
 ...
 */
function resolveReplyChatId(rawChatId, phoneDigits) {
...
export function resolveWhatsAppSenderIdentity(input) {
    ...
    const phoneJid = [input.from, input.chatId, input.author].find((j) => j && !isLidJid(j) && (j.includes("@c.us") || j.includes("@s.whatsapp.net")));
    if (phoneJid) {
