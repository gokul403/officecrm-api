...
import { normalizePhoneDigits, phonesMatchLocal, toLocalPhoneDigits, sendWhatsAppToChatId, } from "./whatsapp.js";
import { getTaskByNumber, saveTaskList, } from "./whatsapp-task-session.js";
const MAX_TOOL_ROUNDS = 4;
const TASK_BOT_TOOLS = [
    {
        name: "list_my_tasks",
...
export async function findProfileByPhoneDigits(phoneDigits) {
    ...
    const result = await pool.query(`SELECT id, full_name, phone FROM profiles
    ...
    if (!matched)
        return null;
    return result.rows.find((r) => r.id === matched.id) ?? null;
}
async function assertAssigned(taskId, profileId) {
    const result = await pool.query(`SELECT 1 FROM task_assignees WHERE task_id = $1 AND profile_id = $2`, [taskId, profileId]);
    return result.rows.length > 0;
}
function formatTaskListMessage(items, rows) {
    if (items.length === 0) {
...
function formatDetailsMessage(toolJson) {
    try {
...
function parseShortCommand(text) {
    const t = text.trim();
    let m = t.match(/^details?\s+(\d+)\s*$/i);
    if (m)
        return { kind: "details", n: Number(m[1]) };
...
async function handleShortCommand(params) {
    ...
    if (!item) {
        await sendWhatsAppToChatId(replyChatId, "No recent numbered list, or that number isn’t valid. Send “list my tasks” first.");
        return;
    }
    if (command.kind === "status") {
        ...
        try {
            ...
            await sendWhatsAppToChatId(replyChatId, `Marked “${parsed.task?.title ?? item.title}” as ${parsed.task?.status ?? command.status}.`);
        }
        catch {
    ...
    if (command.kind === "update") {
        ...
        try {
            ...
            await sendWhatsAppToChatId(replyChatId, `Added update on “${item.title}”: ${command.content}`);
        }
        catch {
...
async function executeTool(call, profileId, phoneLocal) {
    ...
    switch (call.name) {
      ...
      `;
            ...
            let numbered = [];
            if (phoneLocal) {
                numbered = saveTaskList(phoneLocal, rows.map((r) => ({ taskId: r.id, title: r.title })));
            }
            else {
        ...
        case "get_task_details": {
            ...
            const taskResult = await pool.query(`SELECT t.id, t.title, t.description, t.status, t.priority, t.due_date, t.completed_at,
                p.name as project_name
         FROM tasks t
         LEFT JOIN projects p ON p.id = t.project_id
         WHERE t.id = $1`, [taskId]);
            const commentsResult = await pool.query(`SELECT c.content, c.created_at, pr.full_name as author_name
        ...
        case "update_task_status": {
            ...
            if (!taskId || !status)
                return JSON.stringify({ error: "task_id and status required" });
            ...
            const result = await pool.query(`UPDATE tasks SET status = $1, completed_at = $2 WHERE id = $3
         RETURNING id, title, status, priority, due_date, completed_at`, [status, completedAt, taskId]);
        ...
        case "add_task_update": {
            ...
            if (!taskId || !content)
                return JSON.stringify({ error: "task_id and content required" });
            if (!(await assertAssigned(taskId, profileId))) {
                return JSON.stringify({ error: "Task not found or not assigned to you" });
            }
            const result = await pool.query(`INSERT INTO task_comments (task_id, user_id, content)
         VALUES ($1, $2, $3)
         RETURNING id, content, created_at`, [taskId, profileId, content]);
            scheduleTaskEmbedding(taskId);
...
function buildSystemPrompt(userName, ragContext) {
    return [
...
export async function handleIncomingWhatsAppMessage(params) {
    ...
    if (!matchPhoneDigits) {
        ...
        await sendWhatsAppToChatId(replyChatId, "I couldn't resolve your WhatsApp phone number yet. Ask an admin to ensure OpenWA LID→phone is enabled, and that your Team profile phone matches your WhatsApp number (local digits only, e.g. 8921565767).");
        return;
    }
    ...
    if (!profile) {
        ...
        await sendWhatsAppToChatId(replyChatId, "Your WhatsApp number is not linked to an Office CRM profile. Ask an admin to set your phone on the Team page (local number without country code, e.g. 8921565767).");
        return;
    }
    ...
    try {
        ...
        ragContext = retrieved
            .map((t, i) => `${i + 1}. [${t.task_id}] "${t.title}" (${t.status}/${t.priority}) — ${t.content.slice(0, 300)}`)
            .join("\n");
    }
    catch (error) {
    ...
    for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
        ...
        if (result.toolCalls?.length) {
            ...
            const listOnly = result.toolCalls.length === 1 && result.toolCalls[0]?.name === "list_my_tasks";
            let listFormatted = null;
    ...
    await sendWhatsAppToChatId(replyChatId, "I took several steps but could not finish. Please try a shorter request, e.g. “list my tasks” or “mark X as completed”.");
}
