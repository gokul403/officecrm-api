...
function mapProperty(prop: ToolParameterProperty): Record<string, unknown> {
  if (prop.enum?.length) {
    return {
      type: "string",
...
function toolDefsToOpenAI(tools: ToolDef[]): ChatCompletionTool[] {
...
export class AzureOpenAIProvider implements AIProvider {
  ...
  private readonly embedProvider: GeminiProvider;
  ...
  constructor(options?: {
    ...
    const apiVersion =
      options?.apiVersion ?? process.env.AZURE_OPENAI_API_VERSION ?? "2024-10-21";
    this.chatDeployment =
  ...
  async chat(input: ChatInput): Promise<ChatResult> {
    ...
    const content =
      typeof choice.content === "string" && choice.content.length
        ? choice.content
        : null;
