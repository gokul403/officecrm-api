...
function mapProperty(prop: ToolParameterProperty): Schema {
  if (prop.enum?.length) {
    return {
...
function toolDefsToGemini(tools: ToolDef[]): FunctionDeclarationsTool {
  const functionDeclarations: FunctionDeclaration[] = tools.map((tool) => ({
    ...
    parameters: {
      ...
      properties: Object.fromEntries(
        Object.entries(tool.parameters.properties).map(([key, prop]) => [key, mapProperty(prop)])
      ),
  ...
  for (const msg of messages) {
    ...
    if (msg.role === "assistant") {
      ...
      if (msg.providerModelParts?.length) {
        contents.push({
          role: "model",
          parts: msg.providerModelParts as Part[],
        });
        continue;
      }
  ...
  async embed(texts: string[]): Promise<number[][]> {
    ...
    for (const text of texts) {
      const result = await model.embedContent({
        ...
        ...(dims
          ? ({ outputDimensionality: dims } as { outputDimensionality: number })
          : {}),
...
function truncateAndNormalize(values: number[], dims: number): number[] {
  const truncated = values.length > dims ? values.slice(0, dims) : values;
  if (truncated.length !== dims) {
    throw new Error(
      `Gemini embedding length ${truncated.length} does not match expected ${dims} dimensions`
    );
  }
  let sumSquares = 0;
  for (const v of truncated) sumSquares += v * v;
  const norm = Math.sqrt(sumSquares);
  if (!norm || !Number.isFinite(norm)) return truncated;
