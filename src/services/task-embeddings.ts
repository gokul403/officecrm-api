...
export async function buildTaskEmbeddingContent(taskId: string): Promise<string | null> {
  ...
  const commentsResult = await pool.query(
    `SELECT content FROM task_comments
     ...
     LIMIT $2`,
    [taskId, MAX_RECENT_COMMENTS]
  );

  const commentLines = (commentsResult.rows as { content: string }[])
    .reverse()
...
export async function upsertTaskEmbedding(taskId: string): Promise<void> {
  try {
    ...
    await pool.query(
      `INSERT INTO task_embeddings (task_id, content, embedding, updated_at)
       VALUES ($1, $2, $3::vector, now())
       ON CONFLICT (task_id) DO UPDATE
       SET content = EXCLUDED.content,
           embedding = EXCLUDED.embedding,
           updated_at = now()`,
      [taskId, content, vectorLiteral(embedding)]
    );
...
export function scheduleTaskEmbedding(taskId: string): void {
...
export async function retrieveAssignedTasks(
  profileId: string,
  queryText: string,
  topK: number = TOP_K
  ...
  const result = await pool.query(
    `SELECT te.task_id, te.content, t.title, t.status, t.priority, t.due_date,
     ...
     LIMIT $3`,
    [vectorLiteral(queryEmbedding), profileId, topK]
  );

  return result.rows as RetrievedTask[];
}

export async function backfillAllTaskEmbeddings(): Promise<{ total: number; ok: number; failed: number }> {
  const tasks = await pool.query(`SELECT id FROM tasks`);
  let ok = 0;
  let failed = 0;
  for (const row of tasks.rows as { id: string }[]) {
    try {
