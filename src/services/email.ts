...
const senderEmail = process.env.BREVO_SENDER_EMAIL || "gokulkm9@gmail.com";
...
export async function notifyTaskAssignment(
  task: { title: string; description?: string | null; priority: string; due_date?: string | null },
  assignees: { full_name?: string | null; email: string }[]
) {
...
export async function notifyTaskUpdate(
  task: { title: string; description?: string | null; priority: string; status: string; due_date?: string | null },
  assignees: { full_name?: string | null; email: string }[]
) {
  const formattedDueDate = task.due_date
    ? new Date(task.due_date).toLocaleDateString(undefined, { dateStyle: "medium" })
...
export async function notifyLeadAssignment(
  lead: { name: string; company?: string | null; source?: string | null; status: string; notes?: string | null },
...
export async function notifyLeaveApplication(
  leave: { start_date: string; end_date: string; leave_type: string; reason?: string | null },
  employee: { full_name?: string | null; email: string },
  managers: { full_name?: string | null; email: string }[]
) {
  ...
  await sendTransactionalEmail(
    managers.map(m => ({ name: m.full_name || undefined, email: m.email })),
    subject,
    htmlContent
  );
}
...
export async function notifyLeaveStatusChange(
  leave: { start_date: string; end_date: string; leave_type: string; status: string },
  employee: { full_name?: string | null; email: string },
