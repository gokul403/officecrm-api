...
router.post("/team/update-member/:id", requireAuth, requireRole(["admin"]), async (req: AuthenticatedRequest, res: Response) => {
  ...
  let formattedEmail: string | undefined = undefined;
...
router.post("/team/active", requireAuth, requireRole(["admin"]), async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (!userId || active === undefined) return res.status(400).json({ message: "userId and active boolean are required" });
...
router.post("/team/members", requireAuth, requireRole(["admin"]), async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (role !== "manager" && role !== "employee") return res.status(400).json({ message: "role must be manager or employee" });
  if (role === "employee" && !managerId) return res.status(400).json({ message: "managerId is required for employees" });
      ...
      for (let i = 0; i < SAMPLE_TASKS.length; i++) {
        ...
        const taskInsert = await client.query(
          `INSERT INTO tasks (title, description, priority, status, due_date, created_by, completed_at)
           ...
           RETURNING id`,
          [t.title, t.description, t.priority, t.status, dueDate, callerId, completedAt]
        );
