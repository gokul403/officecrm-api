...
function verifyOpenWASignature(rawBody: Buffer, signature: string | undefined, secret: string): boolean {
...
export async function whatsappWebhookHandler(req: Request, res: Response): Promise<void> {
  try {
    ...
    try {
      payload = JSON.parse(rawBody.toString("utf8")) as OpenWAWebhookBody;
    } catch {
