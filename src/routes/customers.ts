...
router.post("/", requireAuth, requireRole(["admin", "manager"]), async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    const result = await pool.query(
      `INSERT INTO customers (name, email, phone, company, address, status, notes, assigned_to, created_by)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
       RETURNING *`,
      [name, email || null, phone || null, company || null, address || null, status || "active", notes || null, assigned_to || null, createdBy]
    );
...
router.get("/:customerId/interactions", requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  const user = req.user!;
  ...
  try {
    ...
    const interactionsQuery = await pool.query(
      `SELECT i.*, p.full_name as creator_name
...
router.post("/:customerId/interactions", requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  const user = req.user!;
  const { customerId } = req.params;
  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO customer_interactions (customer_id, type, summary, created_by)
       VALUES ($1, $2, $3, $4)
       RETURNING *`,
      [customerId, type || "note", summary, user.id]
    );
...
router.delete("/interactions/:id", requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  const user = req.user!;
  const { id } = req.params;
