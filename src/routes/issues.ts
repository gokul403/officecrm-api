...
router.post("/", requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    if (initial_comment && initial_comment.trim()) {
      await pool.query(
        `INSERT INTO issue_comments (issue_id, user_id, content) VALUES ($1, $2, $3)`,
        [createdIssue.id, creatorId, initial_comment]
      );
    }
    ...
    if (assigned_to) {
      ...
      if (profileRes.rows.length > 0) {
        notifyIssueAssignment(
          { title, description, priority: priority || "medium", status: status || "backlog" },
          profileRes.rows[0]
        ).catch(err => console.error("Error sending issue assignment email:", err));
      }
    }
...
router.get("/:issueId/comments", requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  const { issueId } = req.params;
  try {
    const commentsQuery = await pool.query(
      `SELECT c.id, c.content, c.created_at, p.full_name as author_name, p.avatar_url as author_avatar, c.user_id
...
router.post("/:issueId/comments", requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  const { issueId } = req.params;
